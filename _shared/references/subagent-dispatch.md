...
| `BLOCKED` | `blocking_reason: string` (非空、>= 10 文字) | **同アプローチ retry 禁止**、Phase 3 に reset し `blocking_reason` を `plan-review-feedback.json` に書き込む |
...
dev-kickoff の status parser のみ適用。新規 worker 実装はこれに依存しない。

- `"success"` → `DONE`
- `"fail"` → `BLOCKED` （`blocking_reason` は `"Legacy worker returned 'fail'; treat as approach mismatch (rollout heuristic)"`）
- 上記以外 → schema error として journal failure 記録

legacy mapping を適用した entry は `kickoff.json.phases.6_evaluate.iterations[].generator_meta.legacy_mapped: true` を記録。
dev-evaluate は `legacy_mapped == true` のとき `focus_areas` を `blocking_reason` / `concerns[]` から
synthesize **しない**（synthetic reason 文字列で探索範囲を狭めないため、全範囲監査モードに切替）。
