...
### Task 6: 旧 pr-fix skill を削除 — **follow-up issue に分離（このPRでは実施しない）**

**scope 決定（2026-05-30, minimal-coherent）**: pr-fix 削除は dev-flow-doctor
（`analyze-dev-flow-family.sh` の `DEFAULT_FAMILY_SKILLS`、test fixtures、baselines、
SKILL.md/diagnostic-checks）+ `skill-config.json`（family_skills）+ README へ波及する
orthogonal な後始末を伴う。runtime crash ではない（pr-fix は無害な standalone skill として残る）。
よって**この PR から分離し follow-up issue 化**する。

**follow-up issue に含める作業:**
- `rip pr-fix`（SKILL.md, scripts/）
- `dev-flow-doctor`: `analyze-dev-flow-family.sh:52` の family リストから `pr-fix` 除去、
  fixtures/baselines の pr-fix journal の扱い整理、SKILL.md/diagnostic-checks/responsibility-split 更新
- `skill-config.json:57` の `pr-fix` エントリ除去
- README の pr-fix 記載整理

Task 8 で `gh issue create` により follow-up issue を作成する。

---

### Task 7: dev-flow / dev-kickoff の pr-iterate 呼び出し口を更新（minimal — runtime 維持）

**scope（minimal-coherent）**: 新 pr-iterate（goal 駆動・url-or-number 受理）で
dev-flow/dev-kickoff が **runtime で壊れない**ことだけを担保する。dev-flow 自体の
goal 駆動化（single-mode 全面パラダイム移行）は別 effort（spec 参照）なので**やらない**。

**Files（実調査済の対象）:**
- Modify: `dev-flow/references/single-mode.md`（Step 4 lines 83-113 — **最重要**）
- Modify: `dev-flow/SKILL.md`（`--max-iterations` を pr-iterate 呼出から除去）
- Modify: `dev-flow/references/child-split-mode.md`（`Task: pr-iterate ... --max-iterations $MAX`）
- Modify（必要時）: `dev-flow/references/workflow-detail.md`（"max iterations" 記述）,
  `dev-flow/scripts/flow-status.sh`（NEXT_CMD 文字列）
- dev-kickoff: `Skill: pr-iterate $PR_URL` 呼出は **url 受理に戻したため変更不要**。ただし
  `--max-iterations` を渡している箇所があれば除去。

- [ ] **Step 1: 呼び出し口を列挙**

```bash
grep -rn "pr-iterate\|--max-iterations\|init-iterate\|record-iteration\|check-resume\|pr-review\|pr-fix" \
  dev-flow/ dev-kickoff/ --include="*.md" --include="*.sh"
```

- [ ] **Step 2: single-mode.md Step 4 を goal 駆動へ書き換え（最重要）**

`dev-flow/references/single-mode.md` の Step 4（PR iteration workflow、lines 83-113 付近）は、
削除済の `init-iterate.sh` / `record-iteration.sh` と `Skill: pr-review` / `Skill: pr-fix` を
**MANDATORY workflow** として実行指示している（現状 **runtime で壊れる**）。これを新パラダイムへ:
- inline の init/record/pr-review/pr-fix 手順を**全削除**
- 「pr-iterate skill（subagent, `mode: "auto"`）を起動し、PR が approved+CI passed になるまで
  pr-iterate 内蔵の Stop hook ループに委ねる」だけに簡素化。pr-iterate は自己完結でループする
  （`init-iterate`/`record-iteration` の手動駆動は不要）
- pr-iterate に渡す引数から `--max-iterations` を除去（新 pr-iterate は非受理。上限は Stop hook
  の「7手番」で内蔵済）。`$PR_URL` をそのまま渡す形は維持（url-or-number 受理）

- [ ] **Step 3: 他の `--max-iterations` 受け渡しを除去**

`dev-flow/SKILL.md` / `child-split-mode.md` / `flow-status.sh` 等で pr-iterate に
`--max-iterations $MAX` を渡している箇所からフラグを除去（dev-flow 自身の `--max-iterations`
オプション宣言を残すかは任意だが、**pr-iterate へは渡さない**）。"LGTM or max iterations" の
表現は実害なければ温存可。各修正は1ファイルずつ最小変更。

- [ ] **Step 4: 旧参照ゼロを確認**

```bash
grep -rn "init-iterate\|record-iteration\|check-resume\|post-summary" dev-flow/ dev-kickoff/ \
  && echo "STALE REFS REMAIN" || echo "no stale refs"
# pr-iterate へ --max-iterations を渡す箇所が無いことも確認
grep -rn "pr-iterate.*--max-iterations\|--max-iterations.*pr-iterate" dev-flow/ dev-kickoff/ || echo "no max-iter passthrough"
bash tests/run-all-bats.sh
```
Expected: stale 参照なし・pr-iterate への --max-iterations 受け渡しなし・bats PASS（or graceful skip）。
**注意**: `Skill: pr-fix` 参照は single-mode.md Step 4 の書換で消えるが、pr-fix skill 自体は
このPRでは削除しない（Task 6 は follow-up）。dev-flow-doctor の pr-fix 参照は触らない。

- [ ] **Step 5: Commit**

```bash
git add -A
git commit -m "refactor(dev-flow): pr-iterate goal 駆動化に呼び出し口を整合（--max-iterations 除去・single-mode Step4 簡素化）"
```
