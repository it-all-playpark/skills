...
Phase 6: evaluate    — dev-evaluate              (新規, Opus, Task subagent)
...
loop:
  ...
  Phase 6: dev-evaluate  (Task subagent, 別コンテキスト)
...
### 入力（Task subagent に渡す情報）

Evaluator は以下のみを受け取り、dev-implement の実装コンテキストは一切持たない:
...
実装計画書（Markdown）を worktree 内に出力。dev-implement がこれを入力として受け取る。
...
| `dev-kickoff/scripts/update-phase.sh` | 変更 | 新 Phase 対応 |
