...
3b. **GSC データ分離取得**: KPI精度と記事マッチングのため、GSCは **pages-only** と **queries-only** を個別取得し統合する
   ...
   # Combined（strategy_analyzer.py 入力用）
   python3 -c "
   import json
   with open('claudedocs/gsc-report-pages-YYYYMMDD.json') as f:
       pages = json.load(f)
   with open('claudedocs/gsc-report-queries-YYYYMMDD.json') as f:
       queries = json.load(f)
   combined = {'pages': pages.get('rows', []), 'queries': queries.get('rows', [])}
   with open('claudedocs/gsc-report-combined-YYYYMMDD.json', 'w') as f:
       json.dump(combined, f, indent=2)
   "
   ```

   **重要**: KPI（clicks, impressions, avg_ctr, avg_position）は **pages-only レポート** から算出する。
