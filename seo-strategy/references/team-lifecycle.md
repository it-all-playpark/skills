...
Phase 2: Parallel     → 3専門家が並列分析 + クロスドメイン問い合わせ
...
2. **Agent 起動**: 3専門家を Agent ツールで起動（`team_name: "seo-strategy-team"`）
...
4. `kpi_targets` を算出（analysis_guide.md の目安に基づく）
5. `roadmap` を生成（全セクションの priority × codebase_audit severity を考慮）
6. 統合 JSON ドラフトを組み立て
...
| Agent | 割り当て | 用途 |
|---|---|---|
| seo-lead | ~16 ターン | Setup(2) + Synthesize(6) + Devil's Advocate(4x2) + Finalize(2) + Shutdown(2) |
