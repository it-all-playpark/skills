...
test('checkCapabilities: out-of-enum schema_version は required=[] に丸めず SCHEMA_VERSION_UNSUPPORTED', () => {
  const receipt = makeValidReceipt('surfaceproof/1');
  receipt.schema_version = 'surfaceproof/99';
  const result = checkCapabilities(receipt);
  assert.deepEqual(result, { ok: false, reason_code: 'SCHEMA_VERSION_UNSUPPORTED', missing: [] });
});

test('checkCapabilities: schema_version 欠落（空 object 相当）も SCHEMA_VERSION_UNSUPPORTED', () => {
