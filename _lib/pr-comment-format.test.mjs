...
test('buildReviewCommentBody: blocking 0 件で "要修正（blocking）の指摘なし" を含みテーブルヘッダを含まない', () => {
  const body = buildReviewCommentBody({
    pr: 5,
    iteration: 1,
    decision: 'approve',
  ...
  assert.ok(body.includes('✅ 要修正（blocking）の指摘なし'), 'blocking 空時の表示');
  assert.ok(!body.includes('| # | 重大度 |'), 'テーブルヘッダが存在しない (AC-1)');
});

test('buildReviewCommentBody: blocking 2 件（critical 1 / major 1）で件数内訳と番号付き箇条書きが出る (AC-1)', () => {
  const body = buildReviewCommentBody({
    pr: 3,
    iteration: 2,
    decision: 'request-changes',
  ...
  assert.ok(body.includes('要修正（blocking）2 件（critical 1 / major 1）'), '件数内訳を含む');
  assert.ok(!body.includes('| # | 重大度 |'), 'テーブルヘッダが存在しない (AC-1)');
  assert.ok(body.includes('1. 🔴 critical — `src/a.ts:10`'), '1件目の番号付き見出し行を含む');
  assert.ok(body.includes('   - 指摘: null check missing'), '1件目の指摘行を含む');
  assert.ok(body.includes('   - 提案: add null guard'), '1件目の提案行を含む');
  assert.ok(body.includes('2. 🟠 major — `src/b.ts`'), '2件目の番号付き見出し行を含む');
  assert.ok(body.includes('   - 指摘: unused import'), '2件目の指摘行を含む');
});
...
test('buildReviewCommentBody: description / suggestion に | と \\n を含む finding でエスケープされる (AC-4)', () => {
  ...
  assert.ok(!body.match(/\npipe\|char/), '生の | + 改行が行に残らない');
});

test('buildReviewCommentBody: f.file null で見出し行に "場所指定なし"、suggestion null で提案行が出ない', () => {
  const body = buildReviewCommentBody({
    pr: 5,
    iteration: 1,
    decision: 'request-changes',
  ...
  assert.ok(body.includes('1. 🔴 critical — 場所指定なし'), 'file null で場所指定なしが出る');
  assert.ok(body.includes('   - 指摘: some error'), '指摘行を含む');
  assert.ok(!body.includes('   - 提案:'), 'suggestion null で提案行が出ない');
});
...
test('buildReviewCommentBody: f.file あり f.line null でバッククォート付き file のみ', () => {
  ...
  assert.ok(body.includes('1. 🟠 major — `src/foo.ts`'), 'file がバッククォート付きで出る（line 無し）');
...
test('buildReviewCommentBody: f.file あり f.line あり でバッククォート付き file:line', () => {
  ...
  assert.ok(body.includes('1. 🔴 critical — `src/foo.ts:42`'), 'file:line がバッククォート付きで出る');
  assert.ok(body.includes('null check missing'), 'description を含む');
  assert.ok(body.includes('add null guard'), 'suggestion を含む');
});

test('buildReviewCommentBody: blocking 1 件で "要修正（blocking）1 件" が判定行に出る', () => {
  const body = buildReviewCommentBody({
    pr: 5,
    iteration: 1,
    decision: 'request-changes',
  ...
  assert.ok(body.includes('要修正（blocking）1 件'), '件数が判定行に出る');
});
...
test('buildReviewCommentBody: summary と verificationEvidence を渡すと **総評** + **検証根拠** が出る', () => {
  const body = buildReviewCommentBody({
    pr: 3,
    iteration: 1,
    decision: 'approve',
  ...
  assert.ok(body.includes('**総評**: 結論文'), '**総評**: が出る');
  assert.ok(body.includes('**検証根拠**:'), '**検証根拠**: が出る');
  assert.ok(body.includes('- 根拠1'), '箇条書き項目が出る');

  // 判定行の後に総評が来ることを確認（順序検証）
  const summaryIdx = body.indexOf('**総評**:');
  const decisionIdx = body.indexOf('**判定**:');
  assert.ok(decisionIdx < summaryIdx, '判定行より後に **総評** が来る');
});
...
test('buildReviewCommentBody: summary / verificationEvidence 省略時は現行出力と完全一致', () => {
  ...
  assert.ok(!withoutNew.includes('**総評**'), '省略時: **総評** を含まない');
...
// (1) buildReviewCommentBody: minor 2 件 + blocking 0 件 -> blocking 指摘なし + minor が details 内に折りたたまれる (AC-1, AC-2)
test('buildReviewCommentBody: minor 2 件 + blocking 0 件で「要修正（blocking）の指摘なし」と minor の件数行・details が出る (AC-1, AC-2)', () => {
  const body = buildReviewCommentBody({
    pr: 20,
    iteration: 1,
    decision: 'comment',
  ...
  assert.ok(body.includes('✅ 要修正（blocking）の指摘なし'), 'blocking 指摘なしが出る');
  assert.ok(body.includes('**軽微な指摘（minor、自動修正対象外・参考）**: 2 件'), 'minor 件数行に 2 件が出る');
  assert.ok(!body.includes('| # | 重大度 | 場所 | 指摘 | 提案 |'), 'テーブルヘッダは存在しない (AC-1)');
  assert.ok(body.includes('<details><summary>詳細を表示</summary>'), 'minor が <details> で折りたたまれる');
  assert.ok(body.includes('1. 🟡 minor — `src/a.ts:3`'), 'minor 1件目の見出し行が出る');
  ...
  // AC-2: 件数行が <details> の外（先）にあること
  const countIdx = body.indexOf('**軽微な指摘（minor、自動修正対象外・参考）**:');
  const detailsIdx = body.indexOf('<details>');
  assert.ok(countIdx !== -1 && detailsIdx !== -1 && countIdx < detailsIdx, '件数行が <details> より前（外）にある (AC-2)');
});
...
test('buildReviewCommentBody: minor 省略時は minor 表示を含まない（回帰保証）', () => {
  ...
  assert.ok(!body.includes('軽微な指摘'), 'minor 省略時は minor 見出しを含まない');
