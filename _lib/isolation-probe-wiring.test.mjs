...
test('isolation cleanup agent 呼び出しが agentType/schema/label/phase 込みで存在する', () => {
  assert.match(
    src,
    /const isoClean = await trackedAgent\(isolationCleanupPrompt\(WT\),\s*\{\s*agentType:\s*'dev-runner-haiku',\s*schema:\s*ISOLATION_CLEANUP,\s*label:\s*'isolation-cleanup',\s*phase:\s*'Setup'\s*\}\)/,
...
test('isolation cleanup の失敗は fail-open（log のみ・throw しない）', () => {
  ...
  const idx = src.indexOf('const isoClean = await trackedAgent(isolationCleanupPrompt(WT)');
  const probeIdx = src.indexOf('const isoProbe = await trackedAgent(isolationProbePrompt(WT)');
...
test('isolation cleanup は worktree 作成後・probe より前に配置されている', () => {
  const setupIdx = src.indexOf(`, 'Setup(worktree)')`);
  const cleanIdx = src.indexOf('const isoClean = await trackedAgent(isolationCleanupPrompt(WT)');
  const probeIdx = src.indexOf('const isoProbe = await trackedAgent(isolationProbePrompt(WT)');
