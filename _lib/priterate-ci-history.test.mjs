// F1: CI-failed ラウンドが history と終端 post-summary 投稿に反映されることを pin するテスト。
// issue #392 で review⇄fix ループの per-round `post-review#i` 投稿は廃止され（AC-1）、
// 終端の `post-summary` 1 回のみに統合された。反復履歴・全 blocking 詳細は
// buildTerminalSummaryBody の <details> セクション経由で post-summary に集約される。
// アサーション:
//   (1) label==='post-review#1' の agent 呼び出しは存在しない（issue #392 AC-1: per-round 投稿廃止）
...
test('[ci-history] CI-failed ラウンドが per-round 投稿なしで終端レポートに反映される', async () => {
  ...
  // (1) label==='post-review#1' の呼び出しは存在しない（issue #392 AC-1: per-round 投稿廃止）
