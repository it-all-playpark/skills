...
test("[evalseal] (f) (a)（off）と (b)（shadow）の calls 列は 'trust-' 始まり label を除くと完全一致（非干渉実測）", async () => {
  ...
  const labelsA = callsA.filter((c) => !c.label.startsWith('trust-')).map((c) => c.label);
  const labelsB = callsB.filter((c) => !c.label.startsWith('trust-')).map((c) => c.label);
  assert.deepEqual(labelsB, labelsA, `(f) trust-* を除いた calls label 列は (a)/(b) で一致するはず。a=${JSON.stringify(labelsA)} b=${JSON.stringify(labelsB)}`);
});
