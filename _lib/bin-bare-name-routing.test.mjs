...
// [bin]: workflow が使う bare 名は全て bin/ に存在し、bin/ は 18 名に完全一致する。
//
...
test('[bin-bare-name-routing][bin] bin/ は BARE 18 名に完全一致する', () => {
  const actual = readdirSync(binDir).sort();
  const expected = [...BARE].sort();
  assert.deepEqual(actual, expected, `bin/ の内容が期待 18 名と一致しない: actual=${JSON.stringify(actual)}`);
});
