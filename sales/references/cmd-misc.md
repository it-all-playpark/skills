...
- `gbiz get <法人番号>` で最新の公開情報も併せて表示
- `--with-finance` で `gbiz finance <法人番号>` により財務情報（売上・利益・総資産）も取得
- `--with-workplace` で `gbiz workplace <法人番号>` により職場情報（平均年齢・勤続年数）も取得
