  ...
  Stop:
    - type: prompt
      prompt: |
        ...
        - check-ci.sh の最新出力の status が passed または no_checks（CI チェックが無い場合は CI 条件を満たすものとして扱う）
...
3. **必ず次の1行をそのまま出力する**（Stop hook 判定の唯一の根拠）：
   `PR #$pr: verdict=<approved|request-changes|comment>, CI=<passed|failed|pending|no_checks>`
   この行は approved で終了するターンを含め毎ターン出力する。
4. pr-reviewer の decision が `approved` かつ CI が `passed` または `no_checks` なら、これまでの review→fix 経緯を日本語で
