...
プロジェクトの `skill-config.json`（ルートまたは `.claude/`）に `claude-code-tips.output_dir` があれば `--output` のデフォルト値として使用する。`--output` 引数が明示的に渡された場合はそちらを優先。

```jsonc
// .claude/skill-config.json
{
  "claude-code-tips": {
    "output_dir": "news"   // → cc-tips-YYYY-MM-DD.md を news/ に出力
  }
}
```
...
```
Step 1: Parse args → skill-config.json 読込 → 日付範囲計算、出力ファイル名: cc-tips-YYYY-MM-DD.md（親 Opus）
