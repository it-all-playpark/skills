...
| check_targets | ["statistics", "pricing", "versions", "dates", "urls", "entities", "testimonials", "self_claims", "disclosures", "behavioral_claims"] | チェック対象カテゴリ |
| auto_fix_categories | ["statistics", "pricing", "versions", "dates"] | `--fix` 時に自動置換するカテゴリ |
| review_only_categories | ["urls", "entities", "testimonials", "self_claims", "disclosures", "behavioral_claims"] | `--fix` 時に flag のみで human review に回すカテゴリ (フィクションを別のフィクションに書き換えるリスク回避) |
| disclosure_required_mentions | `[]` | disclosures カテゴリで検証する「言及があれば disclosure 必須」エントリ。プロジェクト固有のため利用先で定義する |
| severity_threshold | warning | 報告レベル (error, warning, info) |
| output_dir | claudedocs | レポート出力先 |

### disclosure_required_mentions スキーマ

```jsonc
{
  "blog-fact-check": {
    "disclosure_required_mentions": [
      {
        "name": "Claude Code",
        "trigger_patterns": ["Claude Code", "claude-code"],
        "required_facts": ["月額", "Pro $20", "Max $100", "サブスクリプション", "30,000円"],
...
|-------|-------------|
| `name` | 表示名（レポート用） |
| `trigger_patterns` | 記事内に出現したら disclosure チェックを発動するパターン列。1つでもヒットしたら検証対象 |
| `required_facts` | この製品/サービスを記事で言及するなら本文中に**少なくとも1つ**現れていてほしい disclosure signal（月額・価格・pricing 系のキーワード） |
| `official_pricing_url` | レポート時に「ここを参照して disclosure を追記してください」と提示する URL |
...
| **disclosures** | config の `disclosure_required_mentions` に登録された製品/サービス（例: Claude Code）が記事で言及されているのに、近傍に料金 disclosure（`required_facts`）が一切現れない omission 検出 | trigger pattern grep でヒット箇所を列挙 → 同一記事内に `required_facts` のいずれかが含まれるかを検証。**含まれない場合 error**: 「Claude Code を引き合いに出すなら月額 disclosure が必要」と flag し、`official_pricing_url` を提示 |
| **behavioral_claims** | 「〜すると〜になる」「〜が原因で〜が起きる」「〜は内部で〜を返す」等の動作・因果メカニズム主張で、同一文中に fabrication-signal トークン（HTTP status code / SIG* / 環境変数 / バックティック識別子 / version-pinned 製品 / Claude Code lifecycle 等）を含むもの | `scripts/extract-behavioral-claims.sh` で 5 種の regex (BC1〜BC5) と signal 共起検査の AND で抽出 → flag のみ。**auto-fix 厳禁**（verify せず別の主張に書き換えるとフィクションを別のフィクションにする）。詳細: [behavioral-claims.md](references/behavioral-claims.md) |
...
**Disclosure omission 検出 (disclosures):**
- config の `disclosure_required_mentions` を反復し、各エントリの `trigger_patterns` を MDX 全文に対して grep
- 1件でもヒットしたら、同一 MDX に `required_facts` のいずれかが含まれるか検証
- 含まれない場合 → `error` で flag。レポートに以下を記載:
  - どの trigger（例: "Claude Code"）が何箇所ヒットしたか
  - `official_pricing_url`
  - 「料金 disclosure を追記するか、当該製品名を一般化（"AI コーディングアシスタント" 等）してください」の推奨アクション
- ⚠️ omission（書かれていない）の検出のため、auto-fix は不可。必ず human review に回す

**Behavioral / causal claim 検証 (behavioral_claims):**
- `blog-fact-check/scripts/extract-behavioral-claims.sh <mdx>` を実行し、JSON `{file, claims: [{line, text, pattern_id, signal_tokens, extra_patterns, surrounding_context}]}` を取得
- 検出は「sentence 単位の regex (BC1〜BC5) AND 同一文 fabrication-signal トークン共起」
- 0 件なら問題なし。1 件以上ある場合は orchestrator / writer が**一次ソース**（公式 doc / commit body / 実装ファイル / 実測ログ）で verify
- verify できないものは削除またはヘッジ表現への書き換えを推奨
- ⚠️ **auto-fix 厳禁**。`review_only_categories` に属するため `--fix` でも flag のみ返す
- パターン一覧 / signal トークン / ヘッジ表現例 / 除外ルールは [references/behavioral-claims.md](references/behavioral-claims.md) を参照

`severity_threshold` に基づきフィルタリング。urls / fabrication / disclosures は default で `error` 扱い。
...
| `review_only_categories` | `["urls", "entities", "testimonials", "self_claims", "disclosures", "behavioral_claims"]` |
