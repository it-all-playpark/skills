...
WORKSPACE_NAME=""
# workspaces.json は skill ディレクトリ直下（gitignored）。別置きは SLACK_WORKSPACES_FILE で指定する
CONFIG_FILE="${SLACK_WORKSPACES_FILE:-$(cd "$(dirname "$0")/.." && pwd)/workspaces.json}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "Env: SLACK_WORKSPACES_FILE overrides the workspaces.json path (default: skill directory)."
      exit 0
      ;;
