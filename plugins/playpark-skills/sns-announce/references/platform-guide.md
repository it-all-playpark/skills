...
### X (Twitter)
- **上限**: 280（重み付きカウント。文字数ではない）
  - 東アジア幅 W/F の文字（漢字・かな・全角記号・絵文字の大半）= 2
  - それ以外（ASCII・半角・改行）= 1
  - URL = 長さに関係なく一律 23（t.co 短縮）
- **英語**: 実質 280 文字
- **日本語**: 実質 ~120 字 + URL(23) + ハッシュタグ
- **目安**: 本文100字 + URL + ハッシュタグ2-3個で **270 以下**
- **検証**: `sns-announce-check-length --platform x "<text>"`（`unicodedata.east_asian_width` で計測。`wc -m` は使わない）
...
### Bluesky
- **上限**: 300文字（素の文字数。URL も短縮されない）
- **検証**: `sns-announce-check-length --platform bluesky "<text>"`
