...
Subagent を呼ぶ場合は [`plugins/playpark-core/_shared/references/subagent-dispatch.md`](../../playpark-core/_shared/references/subagent-dispatch.md) の5要素を事前に書き起こす。
...
- **共有可能なロジック**は同一 plugin 内の `_shared/scripts/` を利用・追加。playpark-core の
  `_lib/common.sh` は locator（PATH 上の `bin/journal` をアンカーに解決）経由で source する
...
- [Subagent Dispatch Rules](../../playpark-core/_shared/references/subagent-dispatch.md) - subagent 呼び出し規約
