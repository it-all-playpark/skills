...
#
# Strategy: build a fixture skill tree ($BATS_TEST_TMPDIR/skills/<skill>/scripts/)
# that mirrors the plugin-relative sibling-skill layout refresh_seed_cache.py
# resolves via `Path(__file__).resolve().parents[2]` (REPO_EXPORT_SCRIPT etc):
# copy the real script into $FIX_ROOT/seed-refresh/scripts/ and place stub
# repo-export/repo-commit/repo-issue/repo-pr scripts as siblings under $FIX_ROOT.
# $HOME is still redirected to an isolated sandbox for unrelated lookups (e.g.
# a stubbed `gh`). A stub `gh` is injected via PATH to answer the "latest
# commit date" / "default branch" lookups deterministically. The export stub
# logs its argv and, controlled by env vars, emits `TOKENS_RAW=`/`TOKENS=`
# stdout lines so the token-recording contract can be verified without real
# repomix/network.
...
setup() {
    ...
    FIX_ROOT="$BATS_TEST_TMPDIR/skills"
    mkdir -p "$FIX_ROOT/seed-refresh/scripts"
    cp "$SKILLS_REPO/seed-refresh/scripts/refresh_seed_cache.py" "$FIX_ROOT/seed-refresh/scripts/refresh_seed_cache.py"
    SCRIPT="$FIX_ROOT/seed-refresh/scripts/refresh_seed_cache.py"

    for skill_dir in repo-export repo-commit repo-issue repo-pr; do
        mkdir -p "$FIX_ROOT/$skill_dir/scripts"
    done
    ...
    cat > "$FIX_ROOT/repo-export/scripts/export_repo.py" << 'EOF'
#!/usr/bin/env python3
import os
import sys
...
EOF
    chmod +x "$FIX_ROOT/repo-export/scripts/export_repo.py"

    cat > "$FIX_ROOT/repo-commit/scripts/export_commit.py" << 'EOF'
#!/usr/bin/env python3
import os
import sys
...
sys.exit(0)
EOF
    chmod +x "$FIX_ROOT/repo-commit/scripts/export_commit.py"

    cat > "$FIX_ROOT/repo-issue/scripts/export_issue.py" << 'EOF'
#!/usr/bin/env python3
import sys
args = sys.argv[1:]
out = None
...
sys.exit(0)
EOF
    chmod +x "$FIX_ROOT/repo-issue/scripts/export_issue.py"

    cat > "$FIX_ROOT/repo-pr/scripts/export_pr.py" << 'EOF'
#!/usr/bin/env python3
import sys
args = sys.argv[1:]
out = None
...
sys.exit(0)
EOF
    chmod +x "$FIX_ROOT/repo-pr/scripts/export_pr.py"
