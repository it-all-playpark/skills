...
# detect-and-install は dev-flow plugin の資産。bin/ の bare 名で解決する（未 install 環境は npm install fallback）
if command -v detect-and-install >/dev/null 2>&1; then
    detect-and-install --path . >&2 2>&1 || warn "detect-and-install returned non-zero"
else
