...
代替手段を harness レベルで遮断し probe の意味を保証する）、判断寄り（pr-iterate の fix / Analyze ゲート後の
missing_context 生成）は `dev-runner`（sonnet）が担う（issue の要件抽出は prerun の決定論 analyze + Jev で、sonnet には転写させない）。全 exec-proxy agent の frontmatter には有限の `maxTurns` を設定する
...
| realized-diff（secfloor-unified の files フィールド。Security floor では danger-grep と同一の統合呼び出し経由） | `null` / schema 不一致 | fail-safe（complex floor） | diff 不明時（realized count 欠損）は実効 shape を安全側の complex に倒す |
| redgreen（`_shared/scripts/redgreen-verify.sh`。Evaluate の対象 AC 全ペアを 1 spawn で `redgreen-verify <WT> <T1> <I1> [<T2> <I2> …]` と呼び、`{results:[{index,red,green,…}]}` を引数順で受ける — root object は agent() schema の制約。ペアの入力エラーは当該要素の `reason` に載せて他ペアは続行、exit 2 は全ペアが入力エラーのときだけ。受理 glob: `*.test.mjs` / `*.bats` / `*.test.ts` / `*.test.tsx`。`.test.mjs` と vitest 系は `redgreen.conf` の `test_cmd` 設定時に test_cmd 単一起動、未設定時は `node --test` / `npx vitest run` 直接。playwright `*.spec.ts` は層2 で exit 2 拒否。test_cmd 経路が走らなかったペアは `testcmd_ran:false` + `headdiff`（test_files の HEAD 差分件数）を出力） | `null` / schema 不一致 | fail-safe（inspection 据え置き） | テスト状態不明時は検査済みにしない |
...
| ci-test-display（Validate `tests:'error'` + Final reconcile skipped のときだけ、PR phase の head sha を期待値に ci-final と同じ finalCiPrompt / finalCiVerdict で CI check を読む表示専用 probe） | `null` / `ok:false` / agent throw / headRefOid ≠ 期待 sha / pending / failure / check 0 件 / 期待 sha 無し（probe 自体を起動しない） | fail-open（終端サマリーのテスト欄を「未実行（環境）・未検証」にし結論行で CI 確認を促す。verified のときのみ「✅ green (CI)」） | テスト未実行を ❌ red と表示しない。finalReconcile / merge tier には渡さない — tier を変えるのは ci-final の経路だけ |
...
| pr-create（dev-flow PR phase `pr#<issue>` — `_lib/pr-artifacts.mjs` の `buildCommitMessage` / `buildPrBody` で state（req / plan / ledger / risk hits）から決定論生成した本文を dev-runner-haiku が `.devflow-tmp/commit-msg.txt` / `pr-body.md` へ verbatim 保存し、bare 単文 `git add -A` / `git commit -F` / `git push -u origin HEAD` / `gh pr create --draft --base <base> --head <branch> --body-file` を順に実行。git は `-C` を付けない（cwd は EnterWorktree 済みの worktree）: `git -C <WT>` 形は excludedCommands に一致せず sandbox 内で走り、push は credential helper が `~/.config/gh` / keychain を読めずに、add / commit は `.git` が write deny 下の repo（skills 等）で index.lock を作れずに失敗する（issue #700）。PR body は結論1行 / 変更 / 受入条件 / 設計判断（≤120 字×5）/ 検証 / Closes の上限付き 6 セクション構成（`PR_BODY_MAX_CHARS`）） | `null` / schema 不一致 / agent throw | abort（`need()` 包み — PR 作成失敗のまま継続しない） | commit message / PR body の材料（issue title・type・AC・plan.summary・architecture_decisions・task 一覧）は PR phase 時点で全て state にあり、LLM に diff を読み直させて本文を再生成させる理由がない。agent 側の要約・判断を含めない転写契約は post-comment / commit-ensure と同型（git-commit / git-pr skill は単体起動用に残し dev-flow からは呼ばない） |
| pr-create の中断応答（`pr#<issue>` が手順 1〜4 のどこかで exit 非0 を踏み、`failed_step`（`commit` / `push` / `pr-create`）と `failure_reason`（失敗コマンドの stderr 末尾 1〜3 行 verbatim）を埋めて返した形。`_lib/pr-artifacts.mjs` の `prPhaseFailure` が判定） | `committed === false` / `pr_url` 空文字 / `Number(pr_number)` が正の整数でない、のいずれか | fail-closed（`dev-flow: PR phase 失敗（step: <failed_step>、reason: <failure_reason>）— proxy 応答 pr_url=… pr_number=… committed=…` で即 throw。closes-check・nested `workflow('dev-flow:pr-iterate')` へ進まない。abort handoff の `abort_label` は `pr#<issue>`、`error_msg` に同文。リトライ・別 worktree 退避・force push 等の fallback は持たない） | `need()` は null 判定のみで `{committed:false, pr_number:0, pr_url:""}` を通してしまい、closes-check が fail-open で 1 spawn 無駄に走った後 nested pr-iterate の引数検証（`pr: 正の整数が必要です`）で abort する。その abort は `abort_label:"pr-iterate"` を指し、proxy が踏んだ git / gh の失敗（`index.lock: Operation not permitted`・push 403・`gh pr create` 失敗）は agent transcript を掘らないと分からない。失敗の場所と理由を人間に見せて止めることだけが目的で、worktree の書き込み可否を prerun で判定する話は環境固有として repo 側で吸収しない |
...
| analyze（prerun の script 段。`dev-flow-prerun` → `prerun-analyze.sh` が `analyze-issue N [--repo R] --contract`（issue 取得の bare `gh issue view` 内蔵）を実行し、breaking keyword hit / comments present の 2 理由だけを Jev（`_shared/scripts/jev-classify.sh`、有界判定・`--redact`）に回して `args.setup.analyze` を組む。deps install と並列。Workflow 側は Setup 末尾の analyze ゲート（固有の phase なし）で `buildReqFromContract` の whitelist 検証と 3 条件ゲート（AC 空 / comment_conflicts 非空 / uncertain 非空）のみを行い通常経路の spawn は 0） | prerun の analyze 段失敗（GitHub 到達不能 / JSON 不正）= `analyze.ok:false` / Jev 応答なし / Jev 低確信 / `DEVFLOW_JEV_DISABLE=1` / override だが権限なし | fail-closed（`analyze.ok:false` は needs_clarification（source=analyze_prerun、spawn 0）。Jev の応答なし・低確信・無効は `uncertain[]`、権限なし override / conflict / 低確信 choice は `comment_conflicts[]` に積み、ゲートが引いたときだけ sonnet（`analyze-clarify#N`）を 1 spawn して人間向け missing_context を生成し needs_clarification で終端。whitelist 不合格は prerun 出力の契約違反として throw） | LLM が issue を転写する工程を持たないので provenance 突合・comment_count 突合・scope 切断時の再実行は不要（転写事故の事後対策だった）。決定論スクリプトは意味的矛盾・非互換変更の要否を判定できず、Jev は有界な確率だけを返すので閾値（0.9）で決定論に落とし、確信の無い判定は人間へ返す（incentive-structural: LLM に黙って片方を採らせない） |
