...
| `review_delta` | `.telemetry.iterate_status` を持つ entry の `iterate_history[]`（pr-iterate entry のみが持つ）の round | `rounds_ge2`（iteration ≥ 2 の round 数）/ `delta_rounds`（そのうち `scope == "delta"`）/ `full_rounds_ge2`（scope 欠落は full 扱い）/ `late_blocking_rounds`・`late_blocking_findings`（round ≥ 2 で拾った blocking = late catch）/ `late_blocking_delta_rounds` / `delta_lines.{sum,measured}`。review#i（i ≥ 2）を fix delta に絞った後に late catch がゼロになり lgtm 率だけ上がっていないかを追う。閾値判定は入れない（件数表示のみ。anomaly を出さない） |
...
| `shape_reason_kind` | `shape_reason` の prefix で `safe_floor`（realized count 欠損 / AC 欠落・issue_type 外・breaking）/ `threshold`（`realized N file(s)`）/ `unknown`（キー欠落 = 根拠未記録の旧 entry）に分類。`shape_reason_kind_by_shape` は shape × 種別の交差表 |
...
run-diagnostics --canary ~/.claude/logs/dev-flow-canary/<timestamp>.json
```
