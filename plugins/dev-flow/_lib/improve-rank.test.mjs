...
test('buildImproveIssueBody: core path 接触で canary AC を自動追記', () => {
  const body = buildImproveIssueBody(
    validCand({ target_paths: ['plugins/dev-flow/.claude/workflows/dev-flow.js'] }),
    { hypothesisBlock: '<HYP>' },
  );
  assert.match(body, /dev-flow-canary/);
});

test('IMPROVE_CORE_PREFIXES: plugins/dev-flow/ 配下 4 prefix + tools/ の 5 要素', () => {
  assert.deepEqual(IMPROVE_CORE_PREFIXES, [
...
test('buildImproveIssueBody: 新 prefix（.claude/workflows）で canary AC を追記', () => {
  const body = buildImproveIssueBody(
    validCand({ target_paths: ['plugins/dev-flow/.claude/workflows/dev-flow.js'] }),
    { hypothesisBlock: '<HYP>' },
  );
...
test('buildImproveIssueBody: 新 prefix（_lib）で canary AC を追記', () => {
  const body = buildImproveIssueBody(
    validCand({ target_paths: ['plugins/dev-flow/_lib/merge-tier.mjs'] }),
    { hypothesisBlock: '<HYP>' },
  );
  assert.match(body, /dev-flow-canary/);
});

test('buildImproveIssueBody: 新 prefix（agents）で canary AC を追記', () => {
  const body = buildImproveIssueBody(
    validCand({ target_paths: ['plugins/dev-flow/agents/evaluator.md'] }),
    { hypothesisBlock: '<HYP>' },
  );
  assert.match(body, /dev-flow-canary/);
});

test('buildImproveIssueBody: 新 prefix（.claude/agents）で canary AC を追記', () => {
  const body = buildImproveIssueBody(
    validCand({ target_paths: ['plugins/dev-flow/.claude/agents/evaluator.md'] }),
    { hypothesisBlock: '<HYP>' },
  );
  assert.match(body, /dev-flow-canary/);
});

test('buildImproveIssueBody: 新 prefix（tools/）で canary AC を追記', () => {
  const body = buildImproveIssueBody(
    validCand({ target_paths: ['tools/sync-inlines.mjs'] }),
    { hypothesisBlock: '<HYP>' },
  );
  assert.match(body, /dev-flow-canary/);
});

test('buildImproveIssueBody: 旧 prefix（.claude/workflows/dev-flow.js）は canary AC を追記しない', () => {
  const body = buildImproveIssueBody(
...
test('buildImproveIssueBody: 旧 prefix（_lib/merge-tier.mjs）は canary AC を追記しない', () => {
  const body = buildImproveIssueBody(
    validCand({ target_paths: ['_lib/merge-tier.mjs'] }),
    { hypothesisBlock: '<HYP>' },
  );
  assert.doesNotMatch(body, /dev-flow-canary/);
});
