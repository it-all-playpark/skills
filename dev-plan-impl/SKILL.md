...
### BLOCKED hand-off (Phase 6 → 3 reset, issue #92)

dev-implement worker が `status == "BLOCKED"` を返した場合、dev-kickoff orchestrator は
`plan-review-feedback.json` に **dev-plan-review schema 互換の findings[] 形式に正規化された
generator block** を書き込む（dev-plan-impl の既存規約を変えずに済むよう、文字列ではなく
findings として渡す）。BLOCKED 経路の feedback は以下の追加ルールで識別する:

| 識別キー | 値 |
|---|---|
| `findings[].severity` | `critical` |
| `findings[].dimension` | `approach_mismatch` |
| `findings[].topic` | 元 `blocking_reason` の先頭 60 文字 |
| `findings[].description` | 元 `blocking_reason` の全文 |
| `findings[].suggestion` | "同アプローチでは進行不可。代替設計を立案すること（現アプローチの再試行は禁止）。" |
| top-level `generator_status` (任意) | `"BLOCKED"` |
| top-level `generator_meta.legacy_mapped` (任意) | `true` で legacy `fail` 由来 |

**消費規約**:

- `dimension == "approach_mismatch"` の finding を見つけた場合、**現アプローチをそのまま流用しない**こと。Architecture Decisions を見直し、別の手段（別 framework / 別 module 構成 / 別 API 利用）を採用する
- `generator_meta.legacy_mapped == true` の場合は `blocking_reason` が synthetic placeholder なので、`description` を信用しすぎず、Phase 6 評価結果 / Phase 5 validate 結果 / 直近の `phases.4_implement.errors` を併読して実際の失敗箇所を特定する
- 通常の plan-review finding（`dimension: scope` 等）と BLOCKED 由来 finding（`dimension: approach_mismatch`）が併存する場合、両方反映するが **approach_mismatch を最優先**で解消する

詳細仕様: [`_shared/references/subagent-dispatch.md`](../_shared/references/subagent-dispatch.md#4-値-status-enum),
[`../dev-implement/references/return-contract.md`](../dev-implement/references/return-contract.md)
