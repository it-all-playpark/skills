...
# runs the script with CLAUDE_JOURNAL_DIR pointing at the fixture, and asserts
# the resulting JSON.
...
# ----------------------------------------------------------------------------
# Build fixture: 5 dev-implement entries within window, each carrying a
...
# Here (1+1)/5 = 0.4 > 0.30, so dev-implement should appear in stuck_skills.
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# Out-of-enum return_status strings must bucket as "unknown" (schema error).
# Internal skills are not expected to emit anything outside the 4-value enum;
# anything else is a contract violation that should surface, not be silently
# normalized.
# ----------------------------------------------------------------------------
...
build_entry "dev-implement" "success" "weird-value"   "ex2" > "$FIXTURE_JOURNAL2/ex2.json"
...
if [[ "$SD2" == "null" || -z "$SD2" ]]; then
  fail "out-of-enum fixture: status_distribution present" "missing"
else
  pass "out-of-enum fixture: status_distribution present"
  ...
  if [[ "$unk" == "1" ]]; then
    pass "out-of-enum return_status bucketed as unknown"
  else
    fail "out-of-enum return_status bucketed as unknown" "got: $unk"
  fi
  # total_with_status only counts 4-value enum entries, so just 1 (DONE) here
  if [[ "$tws" == "1" ]]; then
    pass "total_with_status excludes unknown bucket (counts only 4-value enum)"
  else
    fail "total_with_status excludes unknown bucket (counts only 4-value enum)" "got: $tws"
  fi
fi
