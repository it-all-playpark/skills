...
| `vdelta_verdict` | dev-flow telemetry の per-AC 配列 `vdelta_verdicts` を JSON キー単位でパースした各要素 | improved / unchanged / regressed / inconclusive / null に分類（範囲外は unknown）。0 件でも全キー 0 で安全に出力。集計は `skill == "dev-flow"` の構造化キーのみを対象とし、他 skill（調査系 journal 等）の文字列部分一致で誤カウントしない |
...
| **vdelta_unhealthy** | `warn`（`skipped` は insufficient_data） | dev-flow の vdelta_verdicts 全要素における inconclusive+null の割合が `vdelta_unhealthy_rate`（既定 0.50、**placeholder — 実データ蓄積後にキャリブレーション予定**）を超え、かつ総 verdict 数が `vdelta_min_runs`（既定 5、placeholder）以上。総数が min 未満のときは `severity:"skipped", reason:"insufficient_data"` を明示出力し判定しない | veridelta 検証が inconclusive/未取得に偏っている。redgreen verdict フック（.claude/redgreen.conf）の設定状況・veridelta 実行環境を確認。report-only（blocking gate 化しない — INV-10 advisory） |
