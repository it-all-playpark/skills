...
|---|-----------|---------|---------------------|--------------|
| 1 | `skip` | `test\|it\|describe.skip` | `\b(test\|it\|describe)\.skip\b` | recall-fail-to-skip / recall-only-narrowing |
...
content ベースの 6 パターンは **25 件中 0 件の false positive**（誤検出ゼロ）。合成 cheat 文字列に対する
true positive（§4-2）と合わせ、この 6 パターンは高精度。一方でファイル削除ベースの補助パターンは
**単純な「test ファイル削除 = 危険」ルールでは 4/25（16%）の PR で誤検出**が生じ、revert/re-add・
リファクタ由来の削除と cheat 由来の削除を区別する追加ロジック（同一 diff 内で類似テストが
別ファイルへ移動していないか等）なしでは採用に耐えない。
