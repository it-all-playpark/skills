...
```bash
scripts/load-config.sh [--scheduling-url URL] [--document-url URL] [--meeting-url URL]
```

設定をマージして JSON を返す（defaults → skill-config.json → CLI引数）。
...
- 日付は `today`, `yesterday`, `YYYY-MM-DD`, `MM/DD`, `M月D日` をサポート
- 複数イベントがある場合:
  - `--event-index` 指定あり → そのインデックスのイベントを返す
  - 指定なし → JSON配列を返す。**LLMが一覧を表示しユーザーに選択を促す**
- 終日イベントや「総会」「懇親会」など非商談イベントは一覧に含めるが、自動選択はしない

### Step 2: Geminiメモ取得

1. 選択したイベントの `attachments` から Google Docs の `fileId` を抽出（`mimeType` が `application/vnd.google-apps.document`）
2. 添付がある場合:
   ```bash
   scripts/get-gemini-notes.sh <file-id>
   ```
   プレーンテキストを返す（トークン期限切れ時は自動リトライ）
3. 添付がない場合: カレンダー情報のみで進行し、ユーザーに通知する

### Step 3: 議事録生成（`--skip-minutes` でスキップ可）

`references/minutes-template.md` のフォーマットとペルソナ指示に従い、**LLMが議事録を生成**する。
...
1. 設定から以下を取得（Step 0 の config JSON を参照）:
   - `scheduling_url`, `document_url`, `meeting_url`, `sender_name`, `company_name`
2. 宛先メールアドレスはイベントの description から抽出する（`Email :` フィールドなど）
3. 件名は設定の `email_template.subject` テンプレートを使用
4. **LLMが本文を構成**:
   - 宛名（先方の名前 + 様）
   - 挨拶と御礼
...
5. 下書き作成:
   ...
   ```
   JSON で `draft_id` と `url` を返す
6. `open` コマンドでブラウザで開き、ユーザーにURLも合わせて報告する
