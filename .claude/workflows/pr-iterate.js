...
// 結論値リテラル（outcome / iterate_status 等）と journal/pending 呼び出し語彙が同一 prompt に
...
try {
  const journalSaveRes = await trackedAgent(
    `## Objective\npr-iterate 終端の結果データを一時ファイルへ保存する。\n\n`
    + `## Instructions\n`
    + buildJournalSaveInstr({ payload: telemetryHandoff, saveDir: `${isoWt}/.devflow-tmp` })
