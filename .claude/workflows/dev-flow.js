export const meta = {
  name: 'dev-flow',
  description: 'Issue から LGTM まで: 分析(shape判定)→計画→実装(並列/直列)→test green→評価→merge tier→PR→pr-iterate。micro/standard/complex で plan-review・evaluate の深さを切替(complex: plan上限8/eval上限10)。merge は手動。needs_clarification が返ったら呼び出し元が AskUserQuestion で人間に確認し再起動（worktree は保持）',
  phases: [
    { title: 'Setup' },
    { title: 'Analyze' },
    { title: 'Plan' },
...
async function runImplement(p, fixFeedback, tag, extraContext) {
  const results = []
  for (const t of (p.serial ?? [])) {
    const r = await agent(implPrompt(t, fixFeedback, extraContext),
      { agentType: 'implementer', schema: IMPL, label: `${tag}:serial:${t.id}`, phase: 'Implement' })
    if (r) results.push(r)
  }
  const par = (p.parallel ?? []).map((t) => () =>
    agent(implPrompt(t, fixFeedback, extraContext),
...
if (needsCtx.length) {
  ...
  if (stillNeeds.length) {
    ...
    await writeFailureTelemetry({ error_category: 'needs_clarification', error_msg: `implement: ${stillNeeds.length} task が NEEDS_CONTEXT 解消不能で中断（source=implement）`, telemetry: { gate_policy: GATE_POLICY, shape: SHAPE, plan_iter: planIters, eval_iter: 0 }, phase: 'Implement' })
    return {
      status: 'needs_clarification',
      source: 'implement',
      issue: ISSUE,
...
// ============================================================
{
  ...
  if (dhGate.empty === true) {
    ...
    if (dhRetry.empty === true) {
      await writeFailureTelemetry({ error_category: 'empty_diff', error_msg: 'empty-diff gate: 1 回の差し戻し後も working tree が base と一致（issue #215）', telemetry: { gate_policy: GATE_POLICY, shape: SHAPE, plan_iter: planIters, eval_iter: 0 }, phase: 'Security floor' })
...
for (let i = 1; i <= EVAL_PASSES; i++) {
  ...
  // evaluator.md の同期文言（critical_resolutions / security_clearance 契約）は issue #227 で同期済み。
  // 本 prompt の契約を変更する際は evaluator.md も sandbox 外で合わせて更新する（workflow からは編集不可）。
...
return {
  ...
  design_replan_count: designReplanCount,
