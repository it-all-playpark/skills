...
// ============================================================
{
  ...
  if (dhGate.empty === true) {
    ...
    if (dhRetry.empty === true) {
      await writeFailureTelemetry({ error_category: 'empty_diff', error_msg: 'empty-diff gate: 1 回の差し戻し後も working tree が base と一致（issue #215）', telemetry: { gate_policy: GATE_POLICY, shape: SHAPE, plan_iter: planIters, eval_iter: 0 }, phase: 'Validate' })
