...
// ============================================================
// Phase Plan: dev-planner ⇄ plan-reviewer ループ（上限 20）
// ============================================================
phase('Plan')
let plan = null
let feedback = null
...
for (let i = 1; i <= PLAN_MAX; i++) {
  plan = need(await agent(
    `cd ${WT} で作業。issue 要件と${feedback ? 'レビュー指摘' : '初回計画'}に基づき実装計画を立てよ。\n`
    + `requirements: ${JSON.stringify(req)}\n`
    + `testing: ${TESTING}\n`
    + (feedback ? `前回の plan-reviewer findings（これを解消する revise）:\n${JSON.stringify(feedback)}\n` : '')
  ...
  planVerdict = rev
  log(`plan iteration ${i}: ${rev.verdict} (score ${rev.score})`)
  if (rev.verdict === 'pass') break
  feedback = rev.findings
  if (i === PLAN_MAX) {
    throw new Error(`plan unresolved after ${PLAN_MAX} iterations (last verdict: ${rev.verdict})`)
  }
}
