...
async function writeFailureTelemetry({ error_category, error_msg, telemetry, phase, outcome = 'failure' }) {
  ...
  try {
    const journalSaveRes = await trackedAgent(
      ...
      + `## Instructions\n`
      + buildJournalSaveInstr({ payload, saveDir: `${WT}/.devflow-tmp`, fileName: `payload-devflow-${ISSUE}-failure.json` })
...
try {
  const journalSaveRes = await trackedAgent(
    ...
    + `## Instructions\n`
    + buildJournalSaveInstr({ payload: telemetryHandoff, saveDir: `${WT}/.devflow-tmp`, fileName: `payload-devflow-${ISSUE}.json` })
    + `\n## Output format\n{ "saved": boolean, "path": string }\n`
    + `\n## Tools\n使用可: Write のみ（保存先は指示で固定済み — Bash は不要）\n`
    + `\n## Boundary\n作成した一時ファイル以外のファイルを変更しない。git 操作禁止。\n`
    + `\n## Token cap\n120 語以内。`,
