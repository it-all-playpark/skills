...
- **bg-isolation guard と isolation probe**: bg 起動セッションが呼び出し元 cwd を worktree へ
  ...
  probe の直前には cleanup を置く: worktree 内 gitignored の `.devflow-tmp/` を
  `git clean -fdx -- .devflow-tmp` で除去し、前 run が残した probe artifact（残っていると Write tool の
  未 Read 上書き拒否により isolation 正常でも `written:false` に倒れる — issue #482）と前 run の
  trust 証跡（`trust-test-latest.json` / `trust-risk-*.json`。残ると当該 run の証跡書き込み失敗時に
  evalseal-seal.mjs が古い green/red を silent に拾う）を同時に消す。cleanup は fail-open
...
- 表現: `_lib/trust-wiring.mjs` の `TRUST_LAYER_CONFIG.effectdelta` enum 値（'shadow' → 'advisory' → 'blocking'）。PR phase の pr-observe（read-only 観測）・post-summary の comment-prepare/comment-observe（write-once 投稿の準備・readback 分類。実 gh 呼び出しは subagent bare 単文）は EvalSeal shadow と同じ shadow 固定パターンで配線する（issue #412, epic #390 Phase 4）。
...
| isolation-cleanup（subagent の bare `git -C <worktree> clean -fdx -- .devflow-tmp` 単文 — probe 直前の run 専用 scratch 除去。dev-flow Setup / pr-iterate Iterate 進入前） | `cleaned:false` / null / schema 不一致 / agent throw | fail-open（警告 log のみ。gate・merge tier・security floor へ影響しない） | 除去に失敗して stale な probe artifact が残れば直後の probe が `written:false` で fail-closed に倒れ、復旧手順（worktree 作り直し）も同一。cleanup 自体を fail-closed にすると、除去対象が無い正常系（新規 worktree）と区別できない失敗で run を落とす（issue #493） |
