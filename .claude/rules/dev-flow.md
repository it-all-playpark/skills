...
| journal-handoff（journal-save（stage1: payload を worktree 内 gitignored `.devflow-tmp/` へ Write tool で verbatim 永続化）→ journal-log（stage2: 検証済みファイルパスのみを finalize command へ渡し pending/ へ格納）の 2 段。canonical は `_lib/journal-handoff.mjs`） | `saved:false` / `validateJournalSavedPath` 不合格 / `logged:false` / `null` / schema 不一致 / agent throw | fail-open（telemetry が pending/ に届かなくても run は継続。gate・merge tier・ledger には一切影響しない）。ただし結果は返り値 `journal_log_status` の 3 値 closed enum（`logged` / `save_failed` / `log_failed`）に必ず現れる | telemetry は gate ではないので記録失敗で run を落とさない。一方で silent な欠落は dev-flow-doctor / dev-improve の分母を不定量に減らし、doctor 自身が使う journal が書かれないため検知もできない（issue #494）。fail-open を維持したまま欠落を呼び出し元から観測可能にする |
