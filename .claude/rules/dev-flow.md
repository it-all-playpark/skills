...
- model 既定は frontmatter。override は `dev-implement-fable` のみ（fable 上限の null → `fallbackModel: 'opus'` で再試行・run sticky）。品質ゲート agent には渡さない — gate の判定モデルを credit で黙って変えない
- merge は常に人間（全 tier）
