...
### 出力言語・簡潔性（description / suggestion / summary）

自然文フィールドは**日本語で書く**（コード識別子・パス・コマンド・エラーメッセージ引用は原文のまま）。
未解消 findings は Evaluate phase へ concerns として引き継がれ、終端サマリーのテーブルに
そのまま表示される。1 件につき**「事実 → 影響 → 推奨対応」だけを 200 字以内目安**で書き、
前置き・経緯の再説明・同内容の言い換えを入れない。情報（対象ファイル・具体修正）は削らない。
