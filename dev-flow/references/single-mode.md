...
**CRITICAL: Worktree is MANDATORY in single mode.** The subagent MUST execute Phase 1 (worker subagent via Agent tool when `detect-worker.sh` returns `available:true`, otherwise legacy `git-prepare.sh`) before any implementation. Implementation in the main repository directory is NEVER allowed.

Worker path dispatch is documented in [`dev-kickoff/SKILL.md` → Phase 1 dispatch](../../dev-kickoff/SKILL.md#phase-1-dispatch) (issue #79).
...
Phase 1 has two auto-selected paths (see dev-kickoff/SKILL.md Phase 1 dispatch):
  - Worker subagent: Task(subagent_type: "dev-kickoff-worker", isolation: "worktree") — when detect-worker.sh exits 0
  - Legacy git-prepare.sh — when detect-worker.sh exits 1 (fallback)
