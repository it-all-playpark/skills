...
**CRITICAL: Worktree is MANDATORY in single mode.** The subagent MUST execute Phase 1 (spawn `dev-kickoff-worker` via Agent tool with `isolation: worktree`) before any implementation. Implementation in the main repository directory is NEVER allowed.

Phase 1 dispatch is documented in [`dev-kickoff/SKILL.md` → Phase 1 dispatch](../../dev-kickoff/SKILL.md#phase-1-dispatch).
...
Phase 1 spawns the dev-kickoff-worker subagent via Agent(subagent_type: "dev-kickoff-worker", isolation: "worktree").
