...
| **urls** | 記事内に貼られた `https?://...` 全て | `curl -sI -o /dev/null -w "%{http_code}"` で 200 以外を error 化 |
| **entities** | 「ある美容室では」「サロンA社では」等、実在しそうに見える事業者描写 | regex で signal を検出し writer に verify を要求（実在 verify できないなら削除/書き換え提案） |
| **testimonials** | testimonial 引用ブロック (`> 「...」` + 「担当者は」「お客様は」「と振り返ります」等) | regex で signal を検出し、引用元の実在性を要 verify |
| **self_claims** | 「弊社では〜を運用しています」「playpark では〜を活用」等の自社オペレーション主張 | memory `feedback_no_fabricated_self_workflow` に従い、verify されていない主張は削除/書き換え |
...
**URL liveness (urls):**
- 記事から URL を抽出し HEAD リクエスト
- 200 以外 (404/410/500/タイムアウト等) は error
- リダイレクトは `-L` で最終ステータスを確認
...
**--fix モード**: カテゴリごとに auto-fix の安全度が違うため挙動を分岐:

| Category | --fix の挙動 |
|----------|------------|
| statistics / pricing / versions / dates | 公式ソースで verify 済みの正しい値で自動置換可 |
| urls | 404/410 は削除候補としてレポート。auto-fix はせず human review |
| entities / testimonials / self_claims | **auto-fix しない**。flag して human review に回す（フィクションを別のフィクションに書き換えるリスクがあるため） |

`--fix` 完了時のサマリーには `auto_fixed` / `flagged_for_review` の件数を分けて報告する。
