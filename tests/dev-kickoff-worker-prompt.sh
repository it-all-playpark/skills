...
# Case 5 (issue #93): body must declare single-mode-only and reject parallel/merge
# parallel / merge mode は撤廃された。"single" 固定であることと、撤廃に対する明示的な abort 規定を要求する
echo "$BODY" | grep -qE '(single mode only|single mode 起源|`single`)' \
    || fail "Case 5a: body must document single-mode-only operation"
echo "$BODY" | grep -qE "(parallel/merge removed in issue #93|parallel.*merge.*撤廃|mode must be ['\"\`]?single)" \
    || fail "Case 5b: body must document parallel/merge removal (issue #93)"
