...
- **telemetry**: dev-flow 完走時に workflow が telemetry handoff JSON（merge_tier / gate_policy / danger_hits / shape /
  ...
  vdelta_fail_open / vdelta_verdicts）を
  ...
  これら 4 キーの journal whitelist 登録・dotfiles Stop hook への転送配線は別 issue（issue #356 記載）で扱う — 本 PR は handoff JSON への到達と統合テストでの検証まで（`vdelta_verdict` の既存 precedent 踏襲）。
