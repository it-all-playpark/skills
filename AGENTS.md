...
- **判断系 leaf は subagent** (`.claude/agents/{dev-planner,plan-reviewer,implementer,evaluator,pr-reviewer,dev-runner}.md`)。
  workflow の `agent()` には effort 引数が無いため、effort:max は subagent frontmatter で固定する。
