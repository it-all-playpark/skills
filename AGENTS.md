...
#### bg job からの起動手順（必須の事前 isolate）

`Workflow` は top-level セッション専用で、workflow script 内から harness の `EnterWorktree` は呼べない。
一方 bg job（background セッション）の harness には「セッション cwd を `.claude/worktrees/` 配下へ
isolate するまで共有チェックアウトへの Write/Edit を拒否する」bg-isolation guard があり、dev-flow の
Setup phase は `EnterWorktree` を使わず `git worktree add` で worktree を作るだけなので、bg job から
素朴に `Workflow({name:'dev-flow', args:<issue>})` を呼ぶと implementer の最初の書き込みが guard に
弾かれて run が失敗する（2026-07 実証、issue #369/#371/#374/#376）。

**bg job から dev-flow を起動する前に、呼び出し側セッション自身が isolate すること**（dev-flow.js の
Setup ロジック自体の変更は不要 — 既存 worktree があれば再利用するため両立する）:

```bash
# 1. dev-flow の命名規則と同一パスで worktree を作る（絶対パス推奨 — 現在の cwd が
#    別 worktree 内だと相対パスは cwd 起点で解決されネストしたゴミ worktree になる）
git worktree add -b feature/issue-<N> <repo>/.claude/worktrees/df-<N> origin/<base>
```
```
EnterWorktree({ path: ".claude/worktrees/df-<N>" })   # 既存 worktree への切替は cwd 固定 bg セッションでも成功する
Workflow({ name: "dev-flow", args: "<N>" })            # Setup が df-<N> を再利用し、以降の書き込みは isolate 済みとして通る
```

worktree パス `.claude/worktrees/df-<ISSUE>` / branch `feature/issue-<ISSUE>` の命名規則は Setup phase
（本ファイル該当箇所）と一致させること。base は明示指定が無ければ Setup と同じ解決順（origin/dev →
origin/HEAD）に合わせる。2026-07-19 に issue #377 で 1 回実証済み（Setup〜Evaluate の 26 agent が
書き込みブロックなしで完走）だが実証回数は少なく、継続して再現性を確認すること。

**別問題（未解決）**: 上記手順で書き込みブロックは回避できるが、PR phase（`Skill: git-pr` 呼び出し）が
Auto Mode の安全分類器に「no verified implementation work」等の理由でブロックされるケースが観測されて
いる（issue #377 run）。これは worktree isolation とは別レイヤーの問題で、bg job から dev-flow を
完全自律的に PR まで到達させたい場合は追加調査が必要（未着手）。
...
- **telemetry**: dev-flow 完走時に workflow が telemetry handoff JSON（merge_tier / gate_policy / danger_hits / shape /
  ...
  vdelta_fail_open / vdelta_verdicts / duration_seconds / phase_durations）を
