...
/dev-flow <issue>   → Setup → Analyze(shape 判定) → Plan
                      ...
                      → Evaluate → Merge tier → PR → workflow('pr-iterate')
...
- **判断系 leaf は subagent** (`.claude/agents/{dev-planner,plan-reviewer,implementer,evaluator,pr-reviewer,dev-runner,dev-runner-haiku}.md`)。
  ...
  dev-flow.js / pr-iterate.js 冒頭の `QUALITY_MODEL` 定数で一括指定する（現在 `fable` 試験運用中。
  戻すときは定数を `'opus'` に変更）。model を恒久的に別系統へ固定したい leaf には専用 agent 定義
...
- **gate_policy**: trust 昇順の 4 値 enum — `deterministic-only` / `llm-major-advisory`（既定）/ `llm-major-blocking` / `llm-autonomous`。
  ...
  **既定同一挙動** — 既定 `llm-major-advisory` は goal-ledger の laneOf と全アイテムで一致し、非 default policy のみ gating が変わる（enum で境界を滑らせる設計）。
  out-of-enum 値は明示 error（legacy fallback / version 分岐なし）。canonical は `_lib/gate-policy.mjs`、dev-flow.js への inline コピーは `_lib/gate-policy.sync.test.mjs` が byte 一致保証。
- **telemetry**: `journal.sh log` に `--merge-tier` / `--gate-policy` / `--danger-hits` を渡すと telemetry object に記録（未指定なら telemetry キー無し）。calibration の原資料。
...
| クラス | 正当化根拠 | 能力依存 | 代表機構 |
|--------|-----------|---------|---------|
| **incentive-structural**（永続・撤去禁止） | 敵対ループの勝利宣言を当事者に self-judge させない incentive 設計 + cold-context moving-target の抑制 | **非依存**（賢いモデルほどシャープな non-convergent nitpick を出すため逆に悪化） | frozen target（planSeen/evalSeen 累積）・既出 findings/feedback 累積・topic-stuck 検出 + relax + early-cutoff・critical-always-blocks + severity floor + append 単調性・hard cap（PLAN/EVAL/GREEN/BLOCK_MAX, last-resort safety net）|
| **blast-radius**（永続） | 不可逆性 / accountability / liability / blast-radius。正確性ではなく当事者性で正当化するため frontier が人間を超えても残る | **非依存** | human merge（accountability/不可逆/values/novelty）・danger-grep on realized diff → security path 強制・seeded SEC + merge tiering HOLD（danger/breaking/不可逆）|
| **capability-bound**（**sunset 対象**） | 現行 LLM judge の信頼性不足（ECE≈39% / FPR≈35%）。モデルが賢くなるほど縮む | **依存** | `gate_policy = llm-major-advisory`（LLM major を blocking にしない distrust）|

**capability-bound の sunset path（必須）**: パラメータ値で表現し再評価トリガを持たせる。
現状唯一の capability-bound 機構 `gate_policy` は —
